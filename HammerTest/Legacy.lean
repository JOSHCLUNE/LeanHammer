/-
Tests for downstream users that have *not* adopted the module system.

This file is deliberately a plain (non-`module`) Lean file that does a plain `import Hammer`, i.e.
exactly what a pre-module-system downstream user writes. Its purpose is to pin down that adopting
the module system inside LeanHammer did not change anything such a user observes.

`HammerTest/ModuleSystem.lean` runs the same checks from a `module`.
-/
import Hammer

set_option maxHeartbeats 1000000

theorem hammerTestInvolutive {α : Type} {f : α → α} (h : ∀ x, f (f x) = x) (a : α) :
    f (f a) = a := h a
...
Downstream users can build on LeanHammer's definitions directly, so check that the names are still
reachable through `import Hammer` and still have the same types. -/
...
`grind`. Those suggestions are only usable if `import Hammer` still transitively provides the
tactics, which is what makes the `public import`s in LeanHammer's modules load-bearing. -/
